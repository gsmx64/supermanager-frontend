...
const create = async (data: IDeviceSystem) => {
  const endpoint = `device-systems`;
  const service = new GenericService<IDeviceSystem>(endpoint);

  return await service.save(data);
};

const update = async (id: number, data: any) => {
  const endpoint = `device-systems/${id}/`;
  const service = new GenericService<IDeviceSystem>(endpoint);
...
const getAll = async (
  ...
  const endpoint = `device-systems/${queryString}`;
  const service = new GenericService<IDeviceSystem[]>(endpoint);
  return await service.getList();
};

const DeviceSystemsService = {
  create,
  update,
  remove,
  get,
...
export default DeviceSystemsService;