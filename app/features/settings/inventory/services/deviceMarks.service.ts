...
const create = async (data: IDeviceMarks) => {
  const endpoint = `/device-marks`;
  const service = new GenericService<IDeviceMarks>(endpoint);

  return await service.save(data);
};

const update = async (id: number, data: any) => {
  const endpoint = `device-marks/${id}/`;
  const service = new GenericService<IDeviceMarks>(endpoint);
...
const getAll = async (
  ...
  const endpoint = `device-marks/${queryString}`;
  const service = new GenericService<IDeviceMarks[]>(endpoint);
  return await service.getList();
};

const DeviceMarksService = {
  create,
  update,
  remove,
  get,
...
export default DeviceMarksService;