...
const create = async (data: IDeviceRAM) => {
  const endpoint = `device-rams`;
  const service = new GenericService<IDeviceRAM>(endpoint);

  return await service.save(data);
};

const update = async (id: number, data: any) => {
  const endpoint = `device-rams/${id}/`;
  const service = new GenericService<IDeviceRAM>(endpoint);
...
const getAll = async (
  ...
  const endpoint = `device-rams/${queryString}`;
  const service = new GenericService<IDeviceRAM[]>(endpoint);
  return await service.getList();
};

const DeviceRAMsService = {
  create,
  update,
  remove,
  get,
...
export default DeviceRAMsService;